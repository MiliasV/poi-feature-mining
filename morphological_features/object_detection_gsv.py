...
PATH_TO_LABELS = os.path.join(REL_PATH + '/object_detection/data','mscoco_label_map.pbtxt')
...
if __name__ == '__main__':
    ...
    IMAGE_SIZE = (12, 8)
    # indexes of categories of interest
    # check category index
    category_ids = [1, 2, 3, 4, 6, 7, 8, 9, 10 ,11, 13, 15, 64]
    imgs = postgis_functions.get_photos_from_db("matched_gsv_ams", 1000)
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph) as sess:
            ...
            #while True:
                ...
                print(scores)
                print(classes)
                detected_index = np.argwhere(scores>0.3)
                print(detected_index)
                print(np.take(classes, detected_index))
