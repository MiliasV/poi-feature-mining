...
def get_processed_lda_reviews_from_db(table, load):
    if load:
        eng_rev_text = load_with_pickle("en_rev_text")
        nl_rev_text = load_with_pickle("nl_rev_text")
        ...
        # Store reviews
        save_with_pickle("en_rev_text", eng_rev_text)
        save_with_pickle("nl_rev_text", nl_rev_text)
        return eng_rev_text, nl_rev_text
...
def train_lda_models(eng_rev, nl_rev, ntopics, passes):
    ...
    corpora.MmCorpus.serialize("en_term_matrix_rev.mm", eng_term_matrix)
    corpora.MmCorpus.serialize("nl_term_matrix_rev.mm", nl_term_matrix)
...
def get_lda_models(eng_rev, nl_rev, ntopics, passes, load, evaluate):
    if load:
        eng_dict = corpora.Dictionary.load("en_dict_rev.pkl")
        nl_dict = corpora.Dictionary.load("nl_dict_rev.pkl")
        lda_eng = models.LdaModel.load("lda_en_rev" + "_" + str(ntopics) + ".model")
        lda_nl = models.LdaModel.load("lda_nl_rev" + "_" + str(ntopics) + ".model")
        eng_term_matrix = corpora.MmCorpus("en_term_matrix_rev.mm")
        nl_term_matrix = corpora.MmCorpus("nl_term_matrix_rev.mm")
    else:
        # Train the LDA model
        lda_eng, lda_nl, eng_dict, nl_dict, eng_term_matrix, nl_term_matrix = train_lda_models(eng_rev, nl_rev, ntopics, passes)
        lda_eng.save("lda_en_rev" + "_" + str(ntopics) + ".model")
        lda_nl.save("lda_nl_rev" + "_" + str(ntopics) + ".model")
        eng_dict.save("en_dict_rev.pkl")
        nl_dict.save("nl_dict_rev.pkl")
    if evaluate:
